...
<head>
  ...
  <title>Redefinir sua senha - Nubbix</title>
</head>
...
export async function seedPasswordResetTemplate(): Promise<void> {
  const existingTemplate = await db
    .select()
    .from(templates)
    .where(
      and(
        eq(templates.context, "forgot.password"),
  ...
  if (existingTemplate.length > 0) {
    ...
    console.log("Template forgot.password já existe, pulando seed.");
    return;
  }
  ...
  await db.insert(templates).values({
    ...
    subject: "Redefinir sua senha - Nubbix",
    body: emailTemplate,
